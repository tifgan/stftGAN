...
dataset = Dataset(np.stack([preprocessed_images[:, :256], tgrads[:, :256]], axis=-1))

time_str = 'commands_md64_tgrads_8k'
...
params_generator['nfilter'] = [8*md, 4*md, 2*md, md, 2]
...
params['net']['shape'] = [256, 128, 2] # Shape of the image
...
real_signals = dataset.get_samples(N=64*4)[:64]
print(real_signals.shape)
fake_signals = np.squeeze(wgan.generate(N=nsamples, z=d2, checkpoint=213000))

#real_signals = ((real_signals[:,:,:,0]-1)*5)
#fake_signals = ((fake_signals[:,:,:,0]-1)*5)
...
scipy.io.savemat('commands_inception_16k_md64_tgrad_213k.mat', {"original": real_signals, "generated": fake_signals})